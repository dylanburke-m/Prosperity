...
# Black-Scholes 
    def get_option_values(self, S, K, TTE):
        ...
        def bs_call(S, K, TTE, s, r=0):        
            d1 = (math.log(S/K) + (r + 0.5 * s**2) * TTE) / (s * TTE**0.5) #Calculating Delta (sensitivity to price changes)
            d2 = d1 - s * TTE**0.5 #risk adjusted probability a call option finishes in the money at expiration
            return S * _N.cdf(d1) - K * math.exp(-r * TTE) * _N.cdf(d2), _N.cdf(d1) #full black-scholes equation. _N normal distribution

        def bs_vega(S, K, TTE, s, r=0): #measuring vega (price sensitivity to a 1% change in IV)
            d1 = d1 = (math.log(S/K) + (r + 0.5*s**2) * TTE) / (s * TTE**0.5)
            return S * _N.pdf(d1) * TTE**0.5 #when bs_vega called upon later it will return this value

        def get_iv(St, K, TTE): #Finding Implied Volatility
            m_t_k = np.log(K/St) / TTE**0.5 #log moneyness/time to expiry. measures how far K is from current S.
            coeffs = [0.27362531, 0.01007566, 0.14876677] # from the fitted vol smile (need to get past data from options)
